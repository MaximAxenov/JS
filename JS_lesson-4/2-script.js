...
for (var index = 0; index < questions.length; index++){
    ...
    if (userAnswer ==-1){
        alert("Возращайтесь, будем вас ждать.")
        break;
    }
    if (userAnswer == questions[index].correct){
        money+=(index+1)*100000;
        alert("Поздравляем вы ответили правильно и получаете за ответ "+((index+1)*100000)+" рублей");
        goals++;
    }
    else {
        wrongAnswer++;
...
alert("Вы ответили правильно на "+goals+" вопросов из "+qoestions.length+" и заработали за игру "+money+" рублей");